...
def lgb(df_train,df_test):
    ...
    t1 = df_train.groupby(kk[0], as_index=False)[kk[1]].agg([
        'count', 'max', 'median', 'min', 'sum', 'std', 'mean'
    ])
    ...
    prediction_2d = lgbm.predict(test_X).reshape(-1, 1)
    df_out['price1'] = Y_scaler.inverse_transform(prediction_2d)
    df_out = df_out[['SaleID','price1']]
    return df_out

if __name__ == "__main__":   
    
    train_file_name = 'E:\\CourseDesign\\aliyun-tianchi-Second-hand-car-transaction-price-main\\used_car_train_20200313.csv'
    test_file_name = 'E:\\CourseDesign\\aliyun-tianchi-Second-hand-car-transaction-price-main\\used_car_testB_20200421.csv'
